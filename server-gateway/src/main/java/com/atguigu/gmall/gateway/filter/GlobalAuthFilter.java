    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        for (String url : authUrlProperties.getLoginAuthUrl()) {
            ...
            if (match){
                ...
                // 3.3、判断用户信息是否正确
                if (info != null){
                    // Redis 中有此用户，exchange 里面的 request 的头会新增一个 userId
                    ServerWebExchange webExchange = userIdTransport(info,exchange);
                    return chain.filter(webExchange);
                }else {
                    // redis 中无此用户【假令牌、token 没有，没登录】
                    // 重定向到登录页
        ...
        // 能走到这儿，即不是静态资源直接放行，也不是必须登录才能访问的，就一普通请求
        // 普通请求只要带了 token，说明可能登录了，只要登录了，就透传用户 id
        String tokenValue = getTokenValue(exchange);
        UserInfo info = getTokenUserInfo(tokenValue);
        if (info != null){
            exchange = userIdTransport(info,exchange);
        }else {
            // 如果前端带了 token，还是没用户信息，代表这个是假令牌
            if (!StringUtils.isEmpty(tokenValue)){
                // 重定向到登录，可以不带 token，要带就得带正确
                return redirectToCustomPage(authUrlProperties.getLoginPage() + "?originUrl=" + uri,exchange);
            }
        }
        return chain.filter(exchange);
...
//                });
    ...
    private ServerWebExchange userIdTransport(UserInfo info,ServerWebExchange exchange){
        if (info != null){
            // 请求一旦发来。所有的请求参数是固定的，不能进行任何修改，只能读取
    ...
    /**
     * 根据 token 的值去 redis 中查到用户信息
     * @param tokenValue
     * @return
     */
    private UserInfo getTokenUserInfo(String tokenValue){
        String json = stringRedisTemplate.opsForValue().get(SysRedisConst.LOGIN_USER + tokenValue);
    ...
    /**
     * 从 cookie 或请求头中取到 token 对应的值
     * @param exchange
     * @return
     */
    private String getTokenValue(ServerWebExchange exchange){
        // 由于前端乱写，到处可能有 【Cookie[token=xxx]】、【Header[token=xxx]】
