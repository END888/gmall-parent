...
public class SeckillBizServiceImpl implements SeckillBizService {
    ...
    public SeckillOrderConfirmVo getSeckillOrderConfirmVo(Long skuId) {
        ...
        if (!StringUtils.isEmpty(json) && !"x".equals(json)){
            ...
            confirmVo.setUserAddressList(userAddressList);
        }
        return confirmVo;
    }
    ...
    private OrderInfo prepareAndSaveOrderInfoForDb(OrderInfo orderInfo) {
        ...
        if (!StringUtils.isEmpty(json) && !"x".equals(json)){
            ...
            redisData.setExpireTime(new Date(System.currentTimeMillis() + 1000 * 60 * 15L));
            ...
            if (StringUtils.isEmpty(redisData.getId())){
                // 远程保存订单
                Long id = orderFeignClient.submitSeckillOrder(redisData).getData();
                redisData.setId(id);
            }
            // 更新到 Redis
            redisTemplate.opsForValue().set(SysRedisConst.SECKILL_ORDER + code,
